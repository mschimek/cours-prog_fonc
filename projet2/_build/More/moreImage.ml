...
let edge_annotation ?(radius=1.) ?(center=P2.o) ~text =
  let radius = 0.15 in
  let circle = P.empty |> P.circle P2.o 0. in
  let width = estimated_width_of_text text in
  let size = 1.5 *. radius /. width in
  let font = { common_font with Font.size = size } in
  ( I.const Color.white
    |> I.cut ~area:(`Anz) circle
  )
...
let draw_arc_flow  ~properties from_point to_point image ~flot ~capacity
	      =
  let draw_color = get_drawing_color properties in
  let fill_color = get_filling_color properties in 
  let linewidth = get_linewidth properties in
  let offset = get_offset properties in 
  let vec_not_normalized = Gg.V2.sub to_point from_point in
  let vec = Gg.(vec_not_normalized |> V2.unit) in
  let normal = Gg.V2.ortho vec in
  let tip_length = 10. *. linewidth in
  let tip_short_length = 7. *. linewidth in 
  let tip_width = 3. *. linewidth in 
  let start_point = Gg.V2.(from_point + offset * vec) in 
  let label_point = Gg.V2.(from_point + half vec_not_normalized 
			   + offset * normal) in 
  let tip_triangle = Gg.V2.(to_point - offset * vec) in
  let base_triangle = Gg.V2.(tip_triangle - tip_short_length * vec) in
  let left_triangle =
  ...
  |> Vg.I.blend filling 
  |> Vg.I.blend drawing 
  |> Vg.I.blend (edge_annotation
		   ~radius:1.  
		   ~center:label_point
		   ~text:( Printf.sprintf "%d/%d" flot  capacity))
...
let draw_vertex_with_number ~properties position ~number image  =
  let node = 
    circle_node ~radius:1.0 ~center:position ~text:(Printf.sprintf "%d" number)
  in
 image
 |> Vg.I.blend node
...
let draw_vertex vertex =
  ...
  in
  let draw_arc (src, dst) =
    let (flot,cap) = match Graph.EMap.find (src,dst) emap with
	| None -> (0,0)
	| Some(f,c) -> (f,c)
    in
    draw_arc_flow
